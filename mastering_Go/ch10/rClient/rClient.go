...
func deleteEndpoint(server string, user User) int {
	userMasrshall, _ := json.Marshal(user)
	u := bytes.NewReader(userMasrshall)
	...
	// Do() 메서드를 사용해 요청을 보내고 서버 응답을 기다린다.
	resp, err := c.Do(req)
	if err != nil {
...
func listEndpoint(server, URL string) (int, string) {
...
func main() {
	...
	fmt.Println("/list")
	HTTPcode, data := listEndpoint(server, "/list")
	fmt.Println(data)
