...
func listHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		fmt.Println("load to db error :", err)
		return
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			fmt.Println("load rows error : ", err)
			return
		}
		fmt.Println("user data : " + user.Username + " || " + user.Password)
		fmt.Fprintf(w, "%s\n", "user data : "+user.Username+" || "+user.Password)
	}
	w.WriteHeader(http.StatusOK)
...
func connectMysql() (*sql.DB, error) {
	conn := fmt.Sprintf("%s:%s@tcp(127.0.0.1:3306)/%s", dbUser, dbPassword, database)
